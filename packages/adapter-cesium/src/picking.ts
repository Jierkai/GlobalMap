 ...
 */
const objectToFeatureMap = new WeakMap<object, any>();
const primitiveToFeatureMap = new Map<string | number | symbol, any>();
 ...
 */
export function bindPickFeature(cesiumObject: any, feature: any): void {
  if (cesiumObject) {
    if (typeof cesiumObject === 'object' || typeof cesiumObject === 'function') {
      objectToFeatureMap.set(cesiumObject, feature);
    } else {
      primitiveToFeatureMap.set(cesiumObject, feature);
    }
  }
}
 ...
 */
export function getBoundPickFeature(cesiumObject: any): any | undefined {
  if (!cesiumObject) return undefined;
  if (typeof cesiumObject === 'object' || typeof cesiumObject === 'function') {
    return objectToFeatureMap.get(cesiumObject);
  }
  return primitiveToFeatureMap.get(cesiumObject);
}
 ...
 * @param cesiumObject - Cesium 对象（Entity、Primitive 或原始 ID）
 */
export function unbindPickFeature(cesiumObject: any): void {
  if (cesiumObject) {
    if (typeof cesiumObject === 'object' || typeof cesiumObject === 'function') {
      objectToFeatureMap.delete(cesiumObject);
    } else {
      primitiveToFeatureMap.delete(cesiumObject);
    }
  }
}
 ...
 */
export function pickSceneFeature(handle: CesiumViewerHandle, windowPosition: Cesium.Cartesian2): any | undefined {
  ...
  if (pickedObject) {
    ...
    return getBoundPickFeature(target) || pickedObject;
  }
  return undefined;
}
 ...
 */
export function drillPickSceneFeatures(handle: CesiumViewerHandle, windowPosition: Cesium.Cartesian2, limit?: number): any[] {
  const viewer = _getInternalViewer(handle);
  if (!viewer) return [];
