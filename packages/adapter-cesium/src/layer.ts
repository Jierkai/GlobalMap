 ...
 * @module layer
 * @description
 * 该模块封装了 Cesium 的图层管理 API，提供统一的函数接口，
 ...
 */
export function bridgeAddImageryLayer(
 ...
 */
export function bridgeRemoveImageryLayer(handle: CesiumViewerHandle, layer: Cesium.ImageryLayer): boolean {
  const viewer = _getInternalViewer(handle);
 ...
 */
export async function bridgeAdd3DTileset(handle: CesiumViewerHandle, url: string, options?: any): Promise<Cesium.Cesium3DTileset | undefined> {
  const viewer = _getInternalViewer(handle);
  if (!viewer) return undefined;
 ...
 * @returns {boolean} 移除成功返回 true，Viewer 无效或移除失败返回 false
 */
export function bridgeRemove3DTileset(handle: CesiumViewerHandle, tileset: Cesium.Cesium3DTileset): boolean {
  const viewer = _getInternalViewer(handle);
  if (!viewer) return false;
 ...
 */
export function bridgeSetTerrainProvider(handle: CesiumViewerHandle, provider: Cesium.TerrainProvider): void {
  const viewer = _getInternalViewer(handle);
  if (viewer) {
 ...
 * @param handle - Cesium Viewer 句柄
 */
export function bridgeRemoveTerrainProvider(handle: CesiumViewerHandle): void {
  const viewer = _getInternalViewer(handle);
  if (viewer) {
 ...
 */
export function bridgeAddDataSource(handle: CesiumViewerHandle, dataSource: Cesium.DataSource): Promise<Cesium.DataSource> | undefined {
  const viewer = _getInternalViewer(handle);
  if (!viewer) return undefined;
 ...
 * @returns {boolean} 移除成功返回 true，Viewer 无效或移除失败返回 false
 */
export function bridgeRemoveDataSource(handle: CesiumViewerHandle, dataSource: Cesium.DataSource): boolean {
  const viewer = _getInternalViewer(handle);
  if (!viewer) return false;
