...
export function createPrimitiveFeatureHandle(
  ...
  let current: ModelFeatureRenderSpec = spec;

  const apply = (patch: Partial<FeatureRenderSpec>): void => {
    current = { ...current, ...patch } as ModelFeatureRenderSpec;
    if (current.kind === 'model') primitive.updateSpec(current);
  };
...
export function createBatchedPrimitiveFeatureHandle(
  ...
  const batcher = getDefaultBatcher();
  const batchId = createBatchKey('feature', id);
