...
describe('ConfigurableGeographicTilingScheme', () => {
  it('should initialize and inherit from GeographicTilingScheme', () => {
    ...
    expect(scheme).toBeInstanceOf(Cesium.GeographicTilingScheme);
  });
  ...
  it('should correctly calculate tile bounding rectangle based on custom origins', () => {
    ...
    // w = -180, e = 0, n = 90, s = -90
    expect(rect.west).toBeCloseTo(Cesium.Math.toRadians(-180), 9);
    expect(rect.east).toBeCloseTo(Cesium.Math.toRadians(0), 9);
    expect(rect.north).toBeCloseTo(Cesium.Math.toRadians(90), 9);
    expect(rect.south).toBeCloseTo(Cesium.Math.toRadians(-90), 9);
  });
  ...
  it('should correctly convert position to tile indices', () => {
    ...
    const carto = Cesium.Cartographic.fromDegrees(-90, 0);
